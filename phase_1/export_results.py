#!/usr/bin/env python3
"""Export compact, paper-ready summaries from Phase 1 TensorBoard runs.
...
DEFAULT_RUNS = ("phase1_songs01", "phase1_songs04", "phase1_songs17")
DEFAULT_INPUT_ROOT = Path("outputs/runs/phase1_data_scaling")
DEFAULT_OUTPUT_ROOT = Path("phase_1/results")
...
def _write_summary_tex(path: Path, rows: Sequence[Mapping[str, Any]]) -> None:
    lines = [
        ...
        r"\toprule",
        r"Run & Songs & Minutes & Best step & Val. loss & High acc. & Low acc. \\",
        r"\midrule",
    ]
    for row in rows:
        lines.append(
...
def _save_figure(fig: plt.Figure, base_path: Path) -> None:
    ...
    fig.savefig(
        ...
        bbox_inches="tight",
        metadata={"Creator": "phase_1/export_results.py", "CreationDate": None, "ModDate": None},
    )
    plt.close(fig)
...
def _plot_validation_loss(runs: Sequence[RunExport], figures_dir: Path) -> None:
    ...
    for index, run in enumerate(runs):
        ...
        color = colors(index)
        ax.plot(steps, values, marker="o", markersize=3.5, label=f"{run.summary['num_songs']} song(s)", color=color)
    ...
    ax.set(xlabel="Training step", ylabel="Combined validation NLL", title="Phase 1 validation loss")
...
def _plot_validation_accuracy(runs: Sequence[RunExport], figures_dir: Path) -> None:
    ...
    for ax, band, title in zip(axes, ("high", "low"), ("High activity", "Low activity")):
        for index, run in enumerate(runs):
            ...
            ax.plot(steps, values, marker="o", markersize=3, label=f"{run.summary['num_songs']} song(s)", color=colors(index))
    ...
    fig.suptitle("Phase 1 validation accuracy")
    fig.tight_layout()
...
def _plot_training_loss(runs: Sequence[RunExport], figures_dir: Path) -> None:
    ...
    for run in runs:
        ...
        ax.plot(steps, values, linewidth=1.3, label=f"{run.summary['num_songs']} song(s)")
    ax.set(xlabel="Training step", ylabel="Mean training loss", title="Phase 1 training loss")
    ax.legend(frameon=False)
    _style_axes(ax)
    fig.tight_layout()
...
def _plot_best_by_gap(runs: Sequence[RunExport], figures_dir: Path) -> None:
    ...
    for row_index, band in enumerate(("high_activity", "low_activity")):
        for run in runs:
            ...
            axes[row_index, 0].plot(x, [float(row["loss"]) for row in rows], marker="o", label=f"{run.summary['num_songs']} song(s)")
            axes[row_index, 1].plot(x, [float(row["acc_top1"]) for row in rows], marker="o", label=f"{run.summary['num_songs']} song(s)")
...
def _plot_qualitative_group(runs: Sequence[RunExport], figures_dir: Path, group_name: str) -> bool:
    ...
    for column, (run, bundle) in enumerate(zip(runs, loaded)):
        ...
        label = f"{run.summary['num_songs']} song(s), step {run.best_step:,}"
...
def _write_readme(output_root: Path, runs: Sequence[RunExport], qualitative: Sequence[str]) -> None:
    ...
    for run in runs:
        ...
        summary_lines.append(
            f"| {row['num_songs']} | {row['best_val_step']:,} | {row['best_combined_loss']:.4f} | "
    ...
    text = f"""# Phase 1 data-scaling results
...
- `figures/`: PNG previews and vector PDFs for the paper. Qualitative figures: {qualitative_text}.
    ...
    *,
    ...
    for name in run_names:
        ...
        try:
            ...
            raise RuntimeError("PyYAML is required to export Phase 1 results") from exc
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    command = "uv run python phase_1/export_results.py --runs {} --input-root {} --output-root {}".format(
